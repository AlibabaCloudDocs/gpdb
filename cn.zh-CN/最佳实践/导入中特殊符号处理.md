...
云数据库AnalyticDB for PostgreSQL支持多种数据导入方法，[数据迁移和同步方案概览](../../../../cn.zh-CN/快速入门/数据迁移和同步/数据迁移和同步方案概览.md#)。
...
您在使用\\COPY语句导入数据时，分隔符的使用方法和OSS高速并行导入时的使用方法一样，而对数据中出现特殊符号的处理方法也和OSS高速并行导入相类似。不同的是COPY语句和`CREATE EXTERNAL TABLE`语句用法略有不同，COPY语句详细用法见[使用COPY命令导入数据](../../../../cn.zh-CN/快速入门/数据迁移和同步/使用 COPY 命令迁移数据到AnalyticDB for PostgreSQL.md#)。

# 使用DTS将RDS for PostgreSQL实时同步数据至AnalyticDB for PostgreSQL {#concept_845282 .concept}
...
-   源RDS for PostgreSQL和目标AnalyticDB for PostgreSQL的数据表必须建有主键。
-   源库需为RDS for PostgreSQL 9.48以上版本。
...
-   源RDS for PostgreSQL需要将`wal_level`参数值改为`logical`，操作步骤如下。

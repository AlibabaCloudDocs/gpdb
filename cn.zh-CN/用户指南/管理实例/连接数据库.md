...
2.  创建实例，具体操作请参见[创建实例](cn.zh-CN/用户指南/管理实例/创建实例.md#)；若已完成创建操作，请单击目标实例ID进入实例详情页。
3.  创建账号，具体操作请参见[设置账号](cn.zh-CN/用户指南/管理实例/设置账号.md#)。

    **说明：** 该账号用于登录DMS，每个实例只能创建一个。

4.  单击实例详情页右上角**登录数据库**。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16845/156272477750990_zh-CN.png)
...
6.  若有页面弹出如下提示，请根据当前情况选择相应设置。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16845/156272477750991_zh-CN.png)


## 命令行客户端 psql {#section_nll_ssr_52b .section}

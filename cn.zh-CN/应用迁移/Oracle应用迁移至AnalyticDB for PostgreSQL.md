...
AnalyticDB for PostgreSQL对Oracle语法有着较好的兼容，本文介绍如何将Oracle应用迁移到AnalyticDB for PostgreSQL。
...
AnalyticDB for PostgreSQL中提供了Orafce插件，该插件提供了一些兼容Oracle的函数。对于这些函数，您无需任何修改转换即可在AnalyticDB for PostgreSQL中使用。
...
对于以下的四个Oracle函数，在AnalyticDB for PostgreSQL中，无需安装Orafce插件就可以提供兼容支持。
...
-   **PL数据类型**
    ...
    -   Nest table
        -   Nest table作为PL变量，可以使用AnalyticDB for PostgreSQL的array类型替换。

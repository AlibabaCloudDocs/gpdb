...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/16862/15578250319770_zh-CN.png)
...
为了做批量Update，需要用到中间表（Stage Table，示例中的source\_table），即为了更新数据临时创建的表。为了更新target\_table的数据，可以先把新数据插入到中间表source\_table中。然后，把新数据通过[COPY命令](../../../../intl.zh-CN/快速入门/数据迁移和同步/使用 COPY 命令迁移数据到AnalyticDB for PostgreSQL.md#)、[OSS外部表](../../../../intl.zh-CN/快速入门/数据迁移和同步/使用 OSS 外部表迁移数据.md#)等方式导入到source\_table。以下示例直接插入了一些数据。
